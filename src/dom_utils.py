...
def _normalize_group_order(
    groups: dict[str, list[Locator]], page
) -> dict[str, list[Locator]]:
    """Reverse radio order within each group if the raw page order is best→worst.

    Checks the visible label of each group's first and last radio using the
    same keywords as the JS extract_all_questions normalization.
    """
    if not groups:
        return groups

    # Get labels for all radios in DOM order (single JS call)
    all_label_data = page.evaluate(
        """() => {
            const radios = Array.from(document.querySelectorAll('input[type="radio"]'));
            return radios.map((r, idx) => {
                const name = r.getAttribute('name') || '';
                const val = r.getAttribute('value') || '';
                // Extract label (simplified version of _extractLabel)
                let label = (r.getAttribute('aria-label') || '').trim();
                if (!label) {
                    const id = r.getAttribute('id');
                    if (id) {
                        try {
                ...
                if (!label) {
                    let sib = r.nextElementSibling;
                    if (sib && (sib.tagName === 'SPAN' || sib.tagName === 'LABEL'))
                        label = sib.textContent.trim();
                }
                if (!label || label.length > 100)
                    label = val;
                return {idx, name, val, label};
            });
        }"""
    )

    # Build lookup: (name, value) → label for matching locators to labels
    # Also build by-index lookup as fallback
    label_by_key: dict[tuple[str, str], str] = {}
    label_by_idx: dict[int, str] = {}
    for item in all_label_data:
        key = (item["name"], item["val"])
        if key not in label_by_key:
            label_by_key[key] = item["label"]
        label_by_idx[item["idx"]] = item["label"]

    # Get DOM index of each radio locator so we can look up its label
    all_radios = page.locator('input[type="radio"]').all()

    def _get_label(radio: Locator) -> str:
        """Get the label of a radio locator by matching name+value."""
        try:
        ...
        try:
            for i, r in enumerate(all_radios):
                if (r.get_attribute("name") or "") == name and (r.get_attribute("value") or "") == val:
                    return label_by_idx.get(i, "")
        except Exception:
            pass
        return ""
    ...
    for key, radios in groups.items():
        if len(radios) < 2:
            normalized[key] = radios
            continue

        first_label = _get_label(radios[0])
        last_label = _get_label(radios[-1])

        first_best = any(kw in first_label for kw in [
            '非常同意', 'Strongly Agree', '强烈推荐', 'Strongly Recommend',
            '一定会', 'Definitely',
        ])
        last_worst = any(kw in last_label for kw in [
            '非常不同意', 'Strongly Disagree', '强烈不推荐', 'Strongly Do Not Recommend',
            '一定不会', 'Definitely Not',
        ])

        if first_best and last_worst:
            normalized[key] = list(reversed(radios))
        else:
            normalized[key] = radios
